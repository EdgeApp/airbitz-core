                     ...
                     tABC_Error *pError)
{
    ...
    // Load the local file:
    ABC_CHECK_RET(ABC_LoginDirFileLoad(&szLocal, pSelf->AccountNum, PIN_FILENAME, pError));
    pLocal = json_loads(szLocal, 0, &error);
    ABC_CHECK_ASSERT(pLocal != NULL && json_is_object(pLocal),
        ABC_CC_JSONError, "Error parsing local PIN JSON");
    e = json_unpack(pLocal, "{s:o, s:s}",
    ...
    ABC_CHECK_RET(ABC_CryptoBase64Decode(szDID, &DID, pError));
    ABC_CHECK_RET(ABC_LoginServerGetPinPackage(DID, LPIN1, &szEPINK, pError));
    pEPINK = json_loads(szEPINK, 0, &error);
    ...
    ABC_CHECK_RET(ABC_CryptoDecryptJSONObject(pEMK_PINK, PINK, &pSelf->MK, pError));
